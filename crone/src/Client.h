...
#ifndef JACKCLIENTTEST_CLIENT_H
#define JACKCLIENTTEST_CLIENT_H
...
public:
    ...
    bool connect(Client<N,M> &sinkClient,
                 unsigned int sourceIdx, unsigned int sinkIdx) {
        if (sourceIdx >= this->getNumSources()) {
            std::cerr << "invalid source index in Client::connect()" << std::endl;
            return false;
        }
        if (sinkIdx >= sinkClient.getNumSinks()) {
            std::cerr << "invalid sink index in Client::connect()" << std::endl;
            return false;
        }
        for (int i=0; i<2; ++i) {
            const unsigned int srcPortIdx = sourceIdx*2 + i;
            const unsigned int dstPortIdx = sinkIdx*2 + i;
            const char* srcPortName = this->getOutputPortName(srcPortIdx);
            const char* dstPortName = sinkClient.getInputPortName(dstPortIdx);
