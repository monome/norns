...
void crone_tape_rec_pause(int paused) { mixerClient->pauseTapeRecord(paused); }
...
void crone_tape_play_pause(int paused) { mixerClient->pauseTapePlayback(paused); }

void crone_tape_play_stop() { mixerClient->stopTapePlayback(); }

void crone_tape_play_loop(int enabled) { mixerClient->setTapeLoop(enabled); }
