...
private:
    ...
    void poke(float phase, float x); // write an audio sample to the buffer
    void poke2(float phase, float x); // interpolated
...
public:
 ...
 private:
    ...
    float phase_rd[2];     // current buffer read phase
    float phase_wr[2];     // current buffer write phase
