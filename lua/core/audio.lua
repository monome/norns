...
function Audio.add_params()
  ...
  params:add_control("output", "output", cs_MAIN_LEVEL)
  params:set_action("output",
    function(x) audio.level_dac(util.dbamp(x)) end)
  params:add_control("input", "input", cs_MAIN_LEVEL)
  params:set_action("input",
    function(x) audio.level_adc(util.dbamp(x)) end)
  local cs_MUTE_LEVEL = cs.new(-math.huge,0,'db',0,-math.huge,"dB")
  params:add_control("monitor", "monitor", cs_MUTE_LEVEL)
  params:set_action("monitor",
    function(x) audio.level_monitor(util.dbamp(x)) end)
  params:add_control("engine", "engine", cs_MAIN_LEVEL)
  params:set_action("engine",
    function(x) audio.level_eng(util.dbamp(x)) end)
  params:add_control("softcut", "softcut", cs_MAIN_LEVEL)
  params:set_action("softcut",
    function(x) audio.level_cut(util.dbamp(x)) end)
  params:add_control("tape", "tape", cs_MUTE_LEVEL)
  params:set_action("tape",
  ...
  params:add_number("headphone", "headphone", 0, 63, 40)
  params:set_action("headphone",
  ...
  params:add_control("rev_eng_input", "rev eng input", cs_DB_LEVEL_9DB)
  params:set_action("rev_eng_input",
  function(x) audio.level_eng_rev(util.dbamp(x)) end)

  params:add_control("rev_cut_input", "rev cut input", cs_DB_LEVEL_9DB)
  params:set_action("rev_cut_input",
  function(x) audio.level_cut_rev(util.dbamp(x)) end)

  params:add_control("rev_monitor_input", "rev monitor input", cs_DB_LEVEL_MUTE)
  params:set_action("rev_monitor_input",
  function(x) audio.level_monitor_rev(util.dbamp(x)) end)

  params:add_control("rev_tape_input", "rev tape input", cs_DB_LEVEL_MUTE)
  params:set_action("rev_tape_input",
  function(x) audio.level_tape_rev(util.dbamp(x)) end)

  params:add_control("rev_return_level", "rev return level", cs_DB_LEVEL)
