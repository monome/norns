...
static int pushline (lua_State *L, int firstline) {
  ...
  if (readstatus == 0) {
    return 0;                               /* no input (prompt will be popped
                                             * by caller) */
  }
...
static int multiline (lua_State *L) {
  for (;; ) {                                             /* repeat until gets a
    ...
    if ( !incomplete(L, status) || !pushline(L, 0) ) {
      lua_saveline(L, line);                              /* keep history */
      return status;                                      /* cannot or should
                                                           * not try to add
                                                           * continuation line
                                                           * */
    }
    lua_pushliteral(L, "\n");                             /* add newline... */
    lua_insert(L, -2);                                    /* ...between the two
                                                           * lines */
...
#else /* if defined(LUA_USE_WINDOWS) */

#define setprogdir(L)           ( (void)0 )

#endif /* if defined(LUA_USE_WINDOWS) */
...
static int pmain (lua_State *L) {
  ...
  if ( (script < argc) &&                /* execute main script (if there is
                                          * one) */
  ...
  if (args & has_i) {                                           /* -i option? */
    doREPL(L);                                                  /* do
                                                                 ...
                                                                 * */
    if ( lua_stdin_is_tty() ) {                                 /* running in
                                                                 * interactive
                                                                 * mode? */
      print_version();
      doREPL(L);                                                /* do
