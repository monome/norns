...
static cairo_surface_t *pngimage;
...
cairo_surface_t *screen_read_png(const char *filename){
	image = cairo_image_surface_create_from_png (filename);
...
void screen_show_png(cairo_surface_t *image, double x, double y){
	int img_w, img_h;
	img_w = cairo_image_surface_get_width (image);
	img_h = cairo_image_surface_get_height (image);
	cairo_set_source_surface (cr, image, x, y);
...
void screen_display_png(const char *filename, double x, double y){
	pngimage = screen_read_png(filename);
	if (pngimage == NULL) { return; }
	screen_show_png(pngimage, x, y);
	cairo_surface_destroy (pngimage);
}

void screen_destroy_png(cairo_surface_t *image){
	cairo_surface_destroy (image);
}
