...
static std::atomic<uint32_t> g_stub_low_frames{0u};

extern "C" {
#include "jack/jack.h"

jack_nframes_t jack_frame_time(jack_client_t *) {
    return static_cast<jack_nframes_t>(g_stub_low_frames.load());
}
}
...
TEST_CASE("jack_client: time is monotonic across threads during wrap") {
    ...
    g_stub_low_frames = 0xFFFFFF00u; // near top
