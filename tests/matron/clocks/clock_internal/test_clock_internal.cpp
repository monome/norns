...
// covers thread loop, tempo publish, and restarts
...
TEST_CASE("internal: default tempo is 120 bpm after init") {
    tests_set_now(0.0);
    // ensure threadless mode so no background thread interferes with other tests
    clock_internal_test_enable_threadless(true);
    clock_internal_init();
    CHECK(clock_internal_get_tempo() == doctest::Approx(120.0));
}

TEST_CASE("internal: set_tempo updates on next tick (not immediately)") {
    tests_set_now(0.0);
    // enable seam before init to avoid starting the background thread
    clock_internal_test_enable_threadless(true);
    clock_internal_init();
...
TEST_CASE("internal: long-run tick stepping still publishes tempo changes") {
    tests_set_now(0.0);
    clock_internal_test_enable_threadless(true);
    clock_internal_init();

    // simulate many ticks (long run); verify the publish path keeps working
...
TEST_CASE("internal: tick counter does not overflow near INT_MAX") {
    tests_set_now(0.0);
    clock_internal_test_enable_threadless(true);
    clock_internal_init();
...
TEST_CASE("internal: manual tick publishes tempo changes and survives time jump") {
    tests_set_now(0.0);
    clock_internal_test_enable_threadless(true);
    clock_internal_init();
...
TEST_CASE("internal: loop rebases after long time jump") {
    ...
    unsigned long long p1 = clock_internal_test_get_published_ticks();
    for (int i = 0; i < 50; ++i) {
...
TEST_CASE("internal: threaded loop survives long time jump and publishes tempo changes") {
    ...
    clock_internal_set_tempo(90.0);
    if (clock_internal_test_get_published_ticks() == pub) {
        CHECK(clock_internal_get_tempo() == doctest::Approx(120.0));
    }
    for (int i = 0; i < 200000 && clock_internal_test_get_published_ticks() == pub; ++i) {
        ;
...
TEST_CASE("internal: restart resets beat and applies latest tempo on next publish") {
    tests_set_now(0.0);
    clock_internal_test_enable_threadless(true);
    clock_internal_init();
    ...
    CHECK(clock_internal_get_beat() > 0.0);
