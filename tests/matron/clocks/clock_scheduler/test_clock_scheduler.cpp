...
TEST_CASE("scheduler: capacity and clear") {
    g_posted.clear();
...
TEST_CASE("scheduler: sleep posts resume after time elapses") {
    g_posted.clear();
    g_now = 0.0;
    clock_scheduler_init();
...
TEST_CASE("scheduler: sync posts resume after passing next grid beat") {
    g_posted.clear();
    g_now = 0.0;
    g_beats = 0.0;
    clock_scheduler_init();
...
TEST_CASE("scheduler: clear all is thread-safe with active background thread") {
    g_posted.clear();
    g_now = 0.0;
    clock_scheduler_init();
...
TEST_CASE("scheduler: reschedule uses current clock beat") {
    g_posted.clear();
    g_now = 0.0;
    g_beats = 0.0;
    clock_scheduler_init();
...
TEST_CASE("scheduler: reset sets sync_clock_beat to 0 for ready sync events") {
    g_posted.clear();
    g_now = 0.0;
...
TEST_CASE("scheduler: rescheduling same thread counts from stored sync beat") {
    g_posted.clear();
    g_now = 0.0;
    g_beats = 0.0;
    clock_scheduler_init();
...
TEST_CASE("scheduler: next-beat boundary math (epsilon/ceil)") {
